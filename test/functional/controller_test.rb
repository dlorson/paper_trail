...
class ControllerTest < ActionController::TestCase
  ...
  test 'disable on create' do
    ...
    assert_equal 0, assigns(:widget).modifications.length
  end
  ...
  test 'disable on update' do
    ...
    post :create, :widget => { :name => 'Flugel' }
    w = assigns(:widget)
    assert_equal 0, w.modifications.length
    put :update, :id => w.id, :widget => { :name => 'Bugle' }
    widget = assigns(:widget)
    assert_equal 0, widget.modifications.length
  end
  ...
  test 'disable on destroy' do
    ...
    widget = assigns(:widget)
    assert_equal 0, PaperTrail::Modification.with_item_keys('Widget', w.id).size
  end
  ...
  test 'create' do
    ...
    assert_equal 153, widget.modifications.last.whodunnit.to_i
    assert_equal '127.0.0.1', widget.modifications.last.ip
    assert_equal 'Rails Testing', widget.modifications.last.user_agent
  end

  test 'update' do
    w = Widget.create :name => 'Duvel'
    assert_equal 1, w.modifications.length
    put :update, :id => w.id, :widget => { :name => 'Bugle' }
    widget = assigns(:widget)
  ...
  test 'destroy' do
    ...
    delete :destroy, :id => w.id
    widget = assigns(:widget)
    versions_for_widget = PaperTrail::Modification.with_item_keys('Widget', w.id)

...
class SerializerTest < ActiveSupport::TestCase
  ...
  context 'YAML Serializer' do
    ...
    should 'work with the default yaml serializer' do
      # Normal behaviour
      assert_equal 2, @fluxor.modifications.length
      ...
      assert_equal @original_fluxor_attributes, YAML.load(@fluxor.modifications[1].object)
      ...
      if RUBY_VERSION.to_f >= 1.9
        assert_equal YAML.dump(@original_fluxor_attributes), @fluxor.modifications[1].object
      end
    end
  end
  ...
  context 'Custom Serializer' do
    ...
    should 'reify with custom serializer' do
      ...
      assert_equal @original_fluxor_attributes, ActiveSupport::JSON.decode(@fluxor.modifications[1].object)
      # This test can't consistently pass in Ruby1.8 because hashes do no preserve order, which means the order of the
      # attributes in the JSON can't be ensured.
      if RUBY_VERSION.to_f >= 1.9
        assert_equal ActiveSupport::JSON.encode(@original_fluxor_attributes), @fluxor.modifications[1].object
      end
    end
    ...
    should 'store object_changes' do
      ...
      assert_equal initial_changeset, @fluxor.modifications[0].changeset
      assert_equal second_changeset,  @fluxor.modifications[1].changeset
    end
  end
